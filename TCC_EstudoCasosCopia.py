...
def parametros_caso(caso_simulacao):
    ...
    match caso_simulacao:
        ...
        case 2: # Turbinas: 2 // Velocidade do Vento = Fixa // Direção do Vento = Fixa // Ângulo Yaw = Fixo // Layout = Variando em X
            ...
            start_layoutx = 500
            increment_layoutx = 300
            num_turbine=2
            
            fi.reinitialize(layout_x=[0.,500], layout_y=[0.,0.])  # Layout do parque
        ...
        case 3: # Turbinas: 2 // Velocidade do Vento = Fixa // Direção do Vento = Fixa // Ângulo Yaw = Variando // Layout = Fixo
            ...
            fi.reinitialize(layout_x=[0.,1000], layout_y=[0.,0.])  # Layout do parque
            
            for i in range(len(fi.layout_x)):       # Laço para nomear as turbinas no formato 'T01' usando o length do layout
                turbine_name.append('T{:02d}'.format(i+1))
            
            vet_yaw_nom = 0*np.ones((situacoes,num_turbine))
            vet_layout_x = start_layoutx*np.ones((situacoes))
...
elif(caso_simulacao==3):
    print('3')
    plt.clf()
    plt.figure(figsize=(10, 6))
    plt.plot(vet_yaw_nom, farm_powers_nom, marker='o', linestyle='-', color='b', label='Potência do Parque')
...
elif(caso_simulacao==2):
    plt.clf()
    plt.figure(figsize=(10, 6))
    plt.plot(vet_layout_x, farm_powers_nom, marker='o', linestyle='-', color='b', label='Potência do Parque')
...
elif(caso_simulacao==5):
    plt.clf()
    plt.figure(figsize=(10, 6))
    plt.plot(vet_layout_y, farm_powers_nom, marker='o', linestyle='-', color='b', label='Potência do Parque')
